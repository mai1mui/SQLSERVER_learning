...
--tạo DB tên AirCraft theo yêu cầu filegroup
...
create table Customer (
	...
	CAddress varchar(50) default ('Unknown'),
...
--sửa đổi bảng customer để thay đổi First_Name:Không cho phép giá trị trùng lặp unique,Không cho phép giá trị NULL
...
insert into Customer (First_Name, Last_Name, CAddress, City, Country, Birthdate)
	values 
...
GO
--sử dụng mệnh đề SELECT … INTO chèn các giá trị từ bảng Customer vào bảng New_Customer
select * into New_Customer from Customer;--Sao chép toàn bộ dữ liệu từ Customer sang bảng mới New_Customer.
	...
	--Nếu New_Customer đã tồn tại và chỉ muốn chèn dữ liệu: 
		...
		--FROM Customer;
	--Nếu chỉ muốn sao chép một số cột: SELECT First_Name, Last_Name, City, Country INTO New_Customer FROM Customer;
	GO

--thêm một cột CState, varchar (10) vào bảng Customer
alter table Customer
	add CState varchar(10);
	GO

--Viết truy vấn để hiển thị thành phố cùng với số lượng Khách hàng có mặt tại đó, trong đó số lượng Khách hàng >=3
...
--hiển thị 30% bản ghi hàng đầu từ bảng Khách hàng
	--sử dụng TOP ... PERCENT để lấy 30% số bản ghi đầu tiên
	select top 30 percent *
		from Customer;
		GO

--Hiển thị First_Name và City của Khách hàng dưới 18 tuổi
...
--Cập nhật các Thành phố cũ từ “Mumbai” thành “Bombay” cho khách hàng dưới 18 tuổi
update Customer
set City	='Bombay'
